  ...
  if (phases.length === 0) {
    return (
      <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
        Add phases to preview the workflow.
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card/70">
      <div className="flex items-center justify-between gap-3 border-b border-border bg-card/80 px-3 py-2">
        <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
          {zoomMode === "fit" ? `Fit ${Math.round(zoom * 100)}%` : `${Math.round(zoom * 100)}%`}
        </span>
      ...
      >
        ...
        >
          ...
          {phases.map((phase, idx) => {
            ...
            const label = phase.label || phase.id || "(unnamed)";
            const backend = phase.aiBackend === "codex" ? "codex instance" : "claude code instance";
              ...
              <g key={phase.id || idx}>
                ...
                >
                  ...
                  <text
                    ...
                    textAnchor="middle"
                  >
                    {phase.type === "checkpoint" ? "manual checkpoint" : "AI runs"}
                  </text>
                </g>
                ...
                {isAuto && (
                  <>
                    ...
                    <text
                      ...
                      textAnchor="middle"
                    >
                      {shortText(`${titleCase(label).toLowerCase()} skill`, 22)}
                    </text>
                    <rect
                      x={agentX}
                      y={y}
