...
import { RELAY_UI_EVENT_TYPES } from "@dev-workflow/protocol";
...
export function removeTaskRunFromState(state, taskId, runId = "") {
  const targetTaskId = String(taskId || "");
  if (!targetTaskId) return {};

  const targetRunId = String(runId || "");
  const removeKeys = Object.keys(state.workflowStatesByRun).filter((stateKey) => {
    const workflowState = state.workflowStatesByRun[stateKey];
    if (workflowState?.taskId !== targetTaskId) return false;
    return !targetRunId || workflowState.runId === targetRunId;
  });

  if (targetRunId) {
    const directKey = getWorkflowStateKey(targetTaskId, targetRunId);
    if (!removeKeys.includes(directKey)) removeKeys.push(directKey);
  }

  if (removeKeys.length === 0) return {};

  const removeKeySet = new Set(removeKeys);
  const omitRemovedKeys = (items) => Object.fromEntries(
    Object.entries(items || {}).filter(([key]) => !removeKeySet.has(key))
  );
  const activeStateKey = getWorkflowStateKey(state.workflowState?.taskId, state.workflowState?.runId);
  const removeActiveState = removeKeySet.has(activeStateKey) || (
    state.activeTask === targetTaskId
    && (!targetRunId || state.workflowState?.runId === targetRunId)
  );

  return {
    activeTask: removeActiveState ? null : state.activeTask,
    workflowState: removeActiveState ? null : state.workflowState,
    workflowStatesByRun: omitRemovedKeys(state.workflowStatesByRun),
    selectedPhaseByRun: omitRemovedKeys(state.selectedPhaseByRun),
    phaseMessagesByRun: omitRemovedKeys(state.phaseMessagesByRun),
    phaseOutputArtifactsByRun: omitRemovedKeys(state.phaseOutputArtifactsByRun),
    phaseInteractionsByRun: omitRemovedKeys(state.phaseInteractionsByRun),
    isStreamingByRun: omitRemovedKeys(state.isStreamingByRun),
    streamingPhaseByRun: omitRemovedKeys(state.streamingPhaseByRun),
    connectionStateByRun: omitRemovedKeys(state.connectionStateByRun),
    debugEventsByRun: omitRemovedKeys(state.debugEventsByRun),
    lastEventAtByRun: omitRemovedKeys(state.lastEventAtByRun),
    lastErrorByRun: omitRemovedKeys(state.lastErrorByRun),
  };
}
...
function attachWorkflowEvents(set, get) {
  ...
  unsubscribeWorkflowEvents = appApi.onWorkflowEvent((msg) => {
    if (!msg) return;
    appendDebugEvent(set, get, msg);

    if (msg.type === RELAY_UI_EVENT_TYPES.tasksRemoveTask) {
      set((state) => removeTaskRunFromState(state, msg.taskId, msg.runId || ""));
      useConfigStore.getState().loadWorkFolders();
    } else if (msg.type === "phase_artifact") {
...
export const useWorkflowStore = create((set, get) => ({
  ...
  async loadTask(taskId, runId) {
    try {
      ...
      const { state, messages, outputArtifacts, interactions } = await appApi.getTaskState(taskId, runId);
  ...
  async connectWorkflow(taskId, workFolder, taskInputs, images, runId, worktreeName, workflowFilename) {
    ...
    try {
      await appApi.startWorkflow({ taskId, workFolder, taskInputs, images, runId, worktreeName, workflowFilename });
    } catch (err) {
  ...
  async approve() {
    ...
    try {
      await appApi.approveWorkflow(activeTask, workflowState?.runId || "");
    } catch (err) {
  ...
  async reject(rejectTo, reason) {
    ...
    try {
      await appApi.rejectWorkflow(activeTask, rejectTo, reason, workflowState?.runId || "");
    } catch (err) {
  ...
  async sendMessage(text, images) {
    ...
    try {
      await appApi.sendWorkflowMessage(activeTask, text, images, workflowState?.runId || "");
    } catch (err) {
  ...
  async resumePhase(phase) {
    ...
    try {
      await appApi.resumeWorkflowPhase(activeTask, phase, workflowState?.runId || "");
    } catch (err) {
  ...
  async retryPhase(phase) {
    ...
    try {
      await appApi.retryWorkflowPhase(activeTask, phase, workflowState?.runId || "");
    } catch (err) {
      const currentState = get().workflowState;
      if (currentState) {
        const reverted = {
  ...
  async pausePhase(phase) {
    ...
    try {
      await appApi.pauseWorkflowPhase(activeTask, phase, workflowState?.runId || "");
    } catch (err) {
      const currentState = get().workflowState;
      if (currentState) {
        const reverted = {
  ...
  async deleteTask(taskId, runId, options = {}) {
    ...
    try {
      appApi.detachWorkflow(targetTaskId, targetRunId);
      await appApi.removeTask(targetTaskId, targetRunId, options);
      ...
      set((state) => removeTaskRunFromState(state, targetTaskId, targetRunId));
  ...
  async removeTaskWorktree(taskId, runId) {
    ...
    try {
      const result = await appApi.removeTaskWorktree(targetTaskId, targetRunId);
