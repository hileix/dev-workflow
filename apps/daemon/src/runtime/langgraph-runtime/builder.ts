...
const WorkflowRuntimeState = Annotation.Root({
  ...
  revisionMessages: Annotation({ default: () => ({}) }),
  logs: Annotation({
...
function getAutoStepIdsBefore(dsl, stepId) {
  const ids = [];
  for (const step of dsl.steps || []) {
    if (step.id === stepId) break;
    if (step.type === "agent" || step.type === "condition") ids.push(step.id);
  }
  return ids;
}
...
async function runAgentStep(step, dsl, adapters, state) {
  ...
  const stepRevisionMessage = String(state.revisionMessages?.[step.id] || "").trim();
  const nextPendingMessages = { ...(state.pendingMessages || {}) };
  const nextPendingImagePaths = { ...(state.pendingImagePaths || {}) };
  nextPendingMessages[step.id] = stepRevisionMessage;
...
function createCheckpointNode(step, dsl, adapters) {
  return async (state) => {
    ...
    const revisionMessages = { ...(state.revisionMessages || {}) };
    if (!approved && decision?.notes) {
      for (const stepId of getAutoStepIdsBefore(dsl, step.id)) {
        pendingMessages[stepId] = decision.notes;
        revisionMessages[stepId] = decision.notes;
      }
    }
...
function createNode(step, dsl, adapters) {
  ...
  if (step.type === "checkpoint") return createCheckpointNode(step, dsl, adapters);
