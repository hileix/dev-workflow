...
import { getBaseDir, readAiApiProfileSync, readAiApiProfilesSync } from "../repositories/config";
import { readManagedSkillContentSync } from "../repositories/skills";
import {
...
import { WORKFLOW_DEBUG_EVENT_TYPES } from "../repositories/debug-events";
import { upsertTask } from "../repositories/workfolders";
import { activeWorkflows, getPhaseContent, normalizeAiBackend, normalizeWorktreeNamingProvider, readPhaseOutputArtifacts, stopActiveWorkflow, WORKTREE_NAMING_PROVIDERS } from "./claude";
import { prepareWorktree, removeWorktree } from "./worktree";
import { buildWorkflowGraphFromDsl, createAppSdkAgentAdapter, createSdkAgentAdapter, FileCheckpointSaver } from "./langgraph-runtime/index";
...
function syncStateShape(langState, currentState) {
  const next = {
    ...
    revisionMessages: langState.revisionMessages || currentState.revisionMessages || {},
...
${context}`;
  try {
    ...
    const backend = normalizeAiBackend(workflow?.runtime?.backend);
